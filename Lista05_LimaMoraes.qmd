...
```{sql connection=con}
SELECT PNAME, FEE, 1.0 * (FEE + 50) / 2 FROM PRESERVE
...
```{sql connection=con}
SELECT MEDIAN(ACRES), MAX(ACRES), MIN(ACRES) FROM PRESERVE
WHERE STATE = 'AZ'
...
```{sql connection=con}
SELECT FEE, ACRES, AVG(0.02 * ACRES) AS NEW_FEE FROM PRESERVE
WHERE STATE = 'AZ'
